...
inline M_EVENT_HANDLER(uart::Controller,receiveHeader)
{
    ...
    {
        checkSum_ = (uint8_t) HEADER_INDICATOR;
...
inline M_EVENT_HANDLER(uart::Controller,receiveLength)
{
    ...
    {
        checkSum_ += rxLength_;
        rxNextEvent_ = &uart::Controller::receiveType1Event;
    }
}
...
inline M_EVENT_HANDLER(uart::Controller,receiveType2)
{
    ...
    else rxNextEvent_ = &uart::Controller::receiveCRCEvent;
}

inline M_EVENT_HANDLER(uart::Controller,receiveData)
{
    rxBuffer_[rxIndex_] = UART_READ;
    checkSum_ += (uint8_t) rxBuffer_[rxIndex_];
    rxIndex_++;
    if (rxIndex_ == rxLength_) rxNextEvent_ = &uart::Controller::receiveCRCEvent;
}

inline M_EVENT_HANDLER(uart::Controller,receiveCRC)
{
    if (checkSum_ == (uint8_t) UART_READ) rxNextEvent_ = &uart::Controller::receiveFooterEvent;
    else rxNextEvent_ = &uart::Controller::receiveHeaderEvent;
}
